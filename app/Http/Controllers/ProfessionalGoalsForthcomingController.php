...
class ProfessionalGoalsForthcomingController extends Controller
{
    public function store_ProfessionalGoalsForthcoming(Request $request)
    {    
        $data = $request->all();   
        $userd = Auth::user()->id;
        ...
        foreach($emp_text as $key => $input) 
        {
            ...
            {    
                if(isset($id[$key]))
                { 
                $score= ProfessionalGoalsForthcoming::where('id',$id[$key])->first();  
